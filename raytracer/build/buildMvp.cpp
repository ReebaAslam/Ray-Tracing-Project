...
#include "../acceleration/KDNode.hpp"
...
void World::build(void) {
  ...
  clock_t begin = clock();
  float r,g,b; 
  for (float x = -8; x <= 8; x += 1) {
    for (float y = -8; y <= 8; y += 1) {
      Sphere* s = new Sphere(Point3D(x, y, 0), 0.2);
      r = (rand()%255)/255.0;
      g = (rand()%255)/255.0;
      b = (rand()%255)/255.0;
  ...
  tree = new KDNode(everything);
  tree = tree->build(geometry, 0);*/
  /*std::cout<<"Number of premitives in the scene = " <<geometry.size()<< std::endl;
  tree = new Grid;
  for (auto shape:  geometry){
    tree->insert(shape);
